...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RylaExperienceManagerComponent)
...
void URylaExperienceManagerComponent::ServerSetCurrentExperience(FPrimaryAssetId ExperienceId)
{
...
void URylaExperienceManagerComponent::StartExperienceLoad()
{
	...
	//   1. 우리는 B_RylaDefaultExperience를 BP로 만든 이유
	...
	// for (const TObjectPtr<URylaExperienceActionSet>& ActionSet : CurrentExperience->ActionSets)
	// {
	// 	if (ActionSet)
	// 	{
	...
	// 아래도, 후일 Bundle을 우리가 GameFeature에 연동하면서 더 깊게 알아보기로 하고, 지금은 앞서 B_RylaDefaultExperience를 로딩해주는 함수로 생각하자
...
void URylaExperienceManagerComponent::OnExperienceLoadComplete()
{
	...
	check(LoadState == ERylaExperienceLoadState::Loading);
	check(CurrentExperience);
	...
	{
		LoadState = ERylaExperienceLoadState::LoadingGameFeatures;
...
void URylaExperienceManagerComponent::OnGameFeaturePluginLoadComplete(const UE::GameFeatures::FResult& Result)
{
...
void URylaExperienceManagerComponent::OnExperienceFullLoadCompleted()
{
	...
	{
		LoadState = ERylaExperienceLoadState::ExecutingActions;
